...
export function SignalTypeForm({
  item,
  onSuccess,
}: {
  item?: SelectSignalType
  onSuccess?: () => void
}) {
  const form = useForm({
    defaultValues: {
    ...
    onSubmit: async ({ value }) => {
      try {
        if (item) {
          await updateSignalType({ data: { id: item.id, name: value.name } })
          toast.success('Тип сигнала изменён')
        } else {
          await addSignalType({ data: { name: value.name } })
          toast.success('Тип сигнала создан')
        }
        onSuccess?.()
      } catch (error) {
