// S3-совместимое хранилище: серверный модуль. Импортируется только из
// обработчиков server-функций (createServerFn), поэтому, как и `@/db`, не
// попадает в клиентский бандл (вырезается tree-shaking'ом). Не давать ссылок
// на эти экспорты на верхнем уровне клиентски-доступных модулей.
//
// Используем глобальные Buffer/crypto (а не именованные импорты из node:*),
// чтобы модуль безопасно проходил bind в клиентском окружении сборки (как pg),
// а затем вырезался из клиентского бандла tree-shaking'ом.
import {
  DeleteObjectCommand,
  GetObjectCommand,
  PutObjectCommand,
